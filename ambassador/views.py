...
class ProductBackendAPIView(APIView):
    # Perfect example of filtering, sorting and paginating products and saving them to a cache
    def get(self, request):

        # Here we will get the products and cache them if they haven't been already
        products = cache.get('products_backend')
        if not products:
            products = list(Product.objects.all())
            cache.set('products_backend', products, timeout=60*30) #30 minutes

        # Here we obtain the string from the URL parameters and filter the products we want from it
        ...
        # Here we can sort the products by its price
...
class LinkAPIView(APIView):
    ...
    def post(self, request):
        ...
        # This commented piece of code is how to retreieve non-paginated data
        # serializer = ProductSerializer(products, many=True)
        # return Response(serializer.data)
